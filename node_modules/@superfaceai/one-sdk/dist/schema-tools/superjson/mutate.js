    ...
    if (typeof targetedProfile === 'string') {
        ...
        if (targetedProfile.defaults) {
            if (!payload.defaults) {
                ...
                // Merge existing with new
                defaults = lib_1.mergeVariables(lib_1.castToNonPrimitive(targetedProfile.defaults) || {}, lib_1.castToNonPrimitive(payload.defaults) || {});
            }
        }
...
function mergeProfileProvider(document, profileName, providerName, payload, fileSystem, environment, logger) {
    var _a;
    var _b, _c, _d;
    var _e = __read(ensureProfileWithProviders(document, profileName, environment, logger), 2), _ = _e[0], targetProfile = _e[1];
    void _;
    ...
    if (typeof profileProvider === 'string') {
        ...
        if (profileProvider.defaults && payload.defaults) {
            // Change
            // Merge existing with new
            defaults = lib_1.mergeVariables(lib_1.castToNonPrimitive(profileProvider.defaults) || {}, lib_1.castToNonPrimitive(payload.defaults) || {});
        }
